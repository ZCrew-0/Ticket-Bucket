...
router.post("/login",function(req,res){
    ...
    loginModel.findByUserEmail(email,function (err,user) {
        if(err) throw err;
        ...
        if(!user){ 
            ...
            if (match){
                console.log(user);
                jwt.sign({user:user},"ticketbucketsecretkey",function(err,token){
                    if(err){
                        res.json({error:"err"});
                    }else{
                        res.json({state:true,
...
router.get("/profile",verifyToken, function(req,res){
    ...
    jwt.verify(req.token,"ticketbucketsecretkey",function(err,data){
